...
private:
	...
	void DrawSceneToProjMap();
	void DrawScreenQuad();
	void DrawProjScreenQuad();
...
bool ShadowsApp::Init()
{
	...
	HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice,
		L"Textures/tree0.dds", 0, 0, &mTreeTexSRV, 0));
...
void ShadowsApp::DrawScene()
{
	...
	mSmap->BindProjAndSetProjRenderTarget(md3dImmediateContext);
	DrawSceneToProjMap();
	md3dImmediateContext->RSSetState(0); // 레스터화기 단계 초기화
	...
	Effects::DisplacementMapFX->SetProjMap(mSmap->ProjMapSRV());
...
void ShadowsApp::DrawSceneToProjMap()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		Effects::DebugTexFX->SetTexture(mTreeTexSRV);
...
// 그림자 맵을 화면에 출력하는 함수
void ShadowsApp::DrawProjScreenQuad()
{
	UINT stride = sizeof(Vertex::Basic32);
	UINT offset = 0;
	...
	{
		Effects::DebugTexFX->SetWorldViewProj(world);
		Effects::DebugTexFX->SetTexture(mSmap->ProjMapSRV());
