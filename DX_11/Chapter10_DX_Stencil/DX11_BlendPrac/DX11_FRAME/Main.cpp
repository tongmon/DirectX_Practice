...
BlendApp::BlendApp(HINSTANCE hInstance)
	...
	mTheta(1.3f * MathHelper::Pi), mPhi(0.4f * MathHelper::Pi), mRadius(80.0f), Boltnum(0), mCyIndexCnt(0)
{
...
bool BlendApp::Init()
{
	...
	// 초당 60 프레임 전기 이미지 읽기
	std::wstring Fname = L"Textures/Bolt", Num;
	for (size_t i = 1; i <= 60; i++) {
		Num = std::to_wstring(i);
		while (Num.length() != 3)
			Num.insert(Num.begin(), L'0');
		Num = Fname + Num + L".bmp";
		HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice,
			Num.c_str(), 0, 0, &mBoltMapSRV[i - 1], 0));
	}
...
void BlendApp::UpdateScene(float dt)
{
	...
	// 초당 60프레임짜리 번개 이미지 변환
	static float base_t = 0.0;
	if (base_t < 0.017f) {
		base_t += dt;
	}
	else {
		Boltnum = (++Boltnum) % 60; // 60이상이 되면 0으로 회귀한다.
		base_t = 0.0f;
	}
...
void BlendApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		Effects::BasicFX->SetDiffuseMap(mBoltMapSRV[Boltnum]);

		md3dImmediateContext->RSSetState(RenderStates::NoCullRS); // 레스터화기 종류 지정
		md3dImmediateContext->OMSetBlendState(RenderStates::PlusBS, blendFactor, 0xffffffff);
		md3dImmediateContext->OMSetDepthStencilState(RenderStates::DepthWriteOffDSS, 0);
		boxTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
		md3dImmediateContext->DrawIndexed(mCyIndexCnt, 0, 0);
...
{
	float sliceCount = 30, stackCount = 30, height = 1.0f, topRadius = 0.5f, bottomRadius = 0.5f;
	...
	for (UINT i = 0; i < ringCount; ++i)
	{
		...
		for (UINT j = 0; j <= sliceCount; ++j)
		{
			...
			vertex.Pos = XMFLOAT3(r * c, y, r * s); // 원 함수에 의해 반지름 r인 원 그리기(물론 sliceCount에 의해)

			// 텍스쳐 입히는 좌표 설정
			vertex.Tex = { (float)j / sliceCount, 1.0f - (float)i / stackCount };
