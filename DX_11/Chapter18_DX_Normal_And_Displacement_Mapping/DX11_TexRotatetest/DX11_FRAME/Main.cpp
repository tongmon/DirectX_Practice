...
// 텍스쳐를 그냥 적용하면 빛을 받아도 밑밑하다.
// 그 이유는 벽돌같이 음영이 있는 텍스쳐는 실제 세계에서 파인 부분은 좀 더 어둡고
// 아닌 부분은 좀 더 밝기 때문이다.
// 이를 반영하기 위해서 텍스쳐 부분마다 법선 벡터를 다르게 주는데 이것을 법선 매핑이라고 한다.
// 따라서 텍스쳐 파일에 더 사실적인 느낌을 주려면 따로 법선 맵을 생성해야한다.
// 텍스쳐 좌표계에서 법선 맵 한 정점의 좌표계는 TBN-좌표계라고 하며 T,B는 텍스쳐 좌표계에서 (u,v) 즉, (x,y)라고 보면 되고
// N은 그 좌표계의 수직이 되는 z축, 법선이 되는 것이다.
// 근데 빛은 세계 공간에서 정의되기에 TBN-좌표계를 세계공간으로 끌어와야한다.
// 따라서 TBN-좌표계 --> 물체 국소 좌표계 --> 세계 좌표계 이러한 축 변환을 거치게 된다.
// 이 축변환은 축의 이동은 고려하지 않고 방향만을 고려하는 것이기 때문에 3x3행렬 변환만이 필요하다.
// 왜냐하면 빛벡터의 세기를 결정하는 것을 벡터의 방향만을 고려하지 반직선처럼 위치까지 고려하지는 않는다.
// 그리고 보통 TBN에서 B는 T와 N의 외적을 통해 획득하지 따로 저장은 잘 안한다고 한다.
// 계속 언급을 안하던 TangetU의 정체가 TBN에서 T를 의미하는 녀석이였다.
...
{
	XMMATRIX TexMat = XMMatrixRotationZ(mTimer.TotalTime() * 0.09);
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		for (int i = 0; i < 10; ++i)
		{
			...
			case RenderOptionsBasic:
				...
				Effects::BasicFX->SetTexTransform(XMLoadFloat4x4(&mTexMatrix));
			...
			case RenderOptionsNormalMap:
				...
				Effects::NormalMapFX->SetTexTransform(XMLoadFloat4x4(&mTexMatrix));
			...
			case RenderOptionsDisplacementMap:
				...
				Effects::DisplacementMapFX->SetTexTransform(XMLoadFloat4x4(&mTexMatrix));
