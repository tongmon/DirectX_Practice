...
struct VertexIn
{
	...
	float2 Tex      : TEXCOORD;
	float3 TangentL : TANGENT; // 국소 좌표계에서 접벡터
};
...
struct VertexOut
{
	...
	float3 TangentW : TANGENT; // 세계 좌표계에서 접벡터
	float2 Tex      : TEXCOORD;
};
...
VertexOut VS(VertexIn vin)
{
	...
	vout.TangentW = mul(vin.TangentL, (float3x3)gWorld); // 접벡터 세계 공간으로 이동
		  ...
		  uniform bool gReflectionEnabled) : SV_Target
{
    ...
    // 접공간 --> 세계 공간으로 변환해주는 TBN 행렬 구축
    float3 N = pin.NormalW;
    float3 T = normalize(pin.TangentW - dot(pin.TangentW, N) * N); // tangentW, unitNormalW 사이의 각을 90도로 만들기 위한 변형
    float3 B = cross(N, T);
    float3x3 TBN = float3x3(T, B, N);
    float3x3 revTBN = transpose(TBN);
	...
	{  
		...
		for(int i = 0; i < gLightCount; ++i)
		{
            ...
            */            
            ...
            // revTBN은 세계 좌표를 접공간(텍스쳐 좌표) 으로 변환하는 행렬이고
            // 지금 이 함수는 이 행렬을 이용하여 원래 normalMapSample 얘를 세계 공간으로 옮겨서
            // 빛 계산을 진행했는데 이번에는 빛과 시점의 기준을 접공간으로 끌어와서
            // 빛 계산을 진행한다.
            // 주의할 점은 normalMapSample 얘의 범위를 [-1,1]로 변형시키는 과정을 함수에 포함 시켜야한다는 것이다.
            ComputeDirectionalLightNormal(gMaterial, gDirLights[i], normalMapSample, toEye,
				revTBN, A, D, S);
