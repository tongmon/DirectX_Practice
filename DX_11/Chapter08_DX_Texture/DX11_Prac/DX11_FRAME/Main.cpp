// 텍스쳐 좌표 변환 연습
// DrawScene 함수에서 알맞은 텍스쳐 좌표 변환을 만들고
// 그것을 fx에 중계해주면 된다.
// 또한 fx를 적절히 수정하여 거울 모드, 경계 모드 등의 효과를
// 만들 수가 있다.
...
private:
	...
	ID3D11ShaderResourceView* mDiffuseMapSRV; // 텍스쳐 리소스 뷰
...
bool CrateApp::Init()
{
	...
	// Texture2D 객체를 생성하면서 셰이더 자원 뷰까지 할당해주는 함수
...
void CrateApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		// 텍스쳐 좌표 변환, 크기 변환 지정
		XMMATRIX TexScale = XMMatrixScaling(3.0f, 3.0f, 0);
		XMMATRIX TexOffset = XMMatrixTranslation(-0.5f, -0.5f, 0);
		XMStoreFloat4x4(&mTexTransform, TexScale * TexOffset); // S * R
		...
		Effects::BasicFX->SetDiffuseMap(mDiffuseMapSRV); // fx파일에 쉐이더 뷰 중계해 텍스쳐 입히기
