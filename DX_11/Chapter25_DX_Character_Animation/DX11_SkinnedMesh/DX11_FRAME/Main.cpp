...
// 3D 애니메이션을 다루는 예제다.
// 애니메이션에는 뼈대가 필요하고 그 위에 입히는 스킨(텍스쳐를 입힐 표피 도형)이 필요하다.
// 뼈대 이동을 보면 일단 각 뼈대마다 단일 키 프레임 값이 존재한다.
// 키 프레임이라는 것은 해당 뼈대의 국소 좌표계에서 크기, 회전, 이동을 모두 정의해놓은 것을 말한다.
// 하댕 뼈대라는 것은 예를 들면 손가락에서 엄지 마지막 뼈 같은 하나의 뼈에서만 정의 된 것이다.
...
// 구해놨을 것이고 스킨의 각 정점에는 이 정점 위치에 영향을 끼치는 뼈대에 대한 인덱스가 저장되어 있다.
// 이 영향을 끼치는 뼈대에 대한 최종 행렬들을 이용하여 (가중치에 따라 각 뼈에 따른 변환 행렬 값이 다르게 계산됨)
// 최종 스킨의 좌표도 구해지게 된다.
