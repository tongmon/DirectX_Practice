...
// SSAO를 어떻게 다루는지 알려주는 예제다.
// 일단 첫번째로 시점에서 해당 장면을 스캔하고 스캔된 정점들의 법선과 깊이 값들이 저장되는
// 텍스쳐를 만든다. 이 텍스쳐를 가지고 1 - 차폐율 결과가 저장된 SSAO 맵을 만들게 되는데
// 이때 p, q, r, v라는 중요한 요소들이 필요하다.
// 기존의 AO는 그냥 정점에서 반구 생성하고 냅다 레이져를 무작위로 몇개 쏴가지고 차폐율을 구했다면
// SSAO는 이렇게 오래걸리는 방식이 아닌 주어진 정보로 차폐율을 추론한다. 그 추론할 때 p, q, r, v가 쓰인다.
// p, q, r, v에 대한 의미는 SSAO.fx에 나오니 참고하고 여기서 r이 중요한데 난 이게 처음에 왜 있는지 싶었다.
// 왜 p에서 쏜 광선 q에 닿은 정점 위치를 쓰지 않고 이상한 r을 쓰게 되는거지? 라는 생각이 들었다.
// 근데 생각해보니 r을 쓸 수 밖에 없다. r은 시점에서 q를 연결하는 벡터에서 중간에 걸리는 물체가 있으면
// 그 물체에 걸리게되는 접점인데 이 접점에 대한 깊이, 법선 정보가 첫번째로 만드는 텍스쳐(법선, 깊이가 담긴 녀석)에
// 담겨져 있기 때문에 r을 쓰는 것이였다. 정작 q는 카메라에서 볼 때 물체에 가려서 보이지도 않는다.
// 즉, 투영창에는 접점 r만이 픽셀로 그려지는 것이다.
// 그래서 r을 이용해 차폐율을 '추론'하는 것이다.
