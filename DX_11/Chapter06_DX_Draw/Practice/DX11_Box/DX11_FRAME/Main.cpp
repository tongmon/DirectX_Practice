// 정점 버퍼 2개로 그림 그리기 연습
// 수정된 중요한 부분은 BuildGeometryBuffers 함수
// DrawScene함수에서 IASetVertexBuffers 사용부분
// 애초에 위치, 색상이 담긴 구조체를 쪼갠 것이기에
// 따로 구조체 생성을 하진 않았다.
...
private:
	// 이게 달라졌다. 원래 mBoxIV였는데 이를 쪼개서 두 개의 정점 버퍼로 나누었다.
	// 이 방식이 어떤지 코드를 읽어보면 알 수 있다.
	ID3D11Buffer* mBoxXYZ; // 위치 버퍼
	ID3D11Buffer* mBoxColor; // 색상 버퍼
...
BoxApp::BoxApp(HINSTANCE hInstance)
	: D3DApp(hInstance), mBoxXYZ(0), mBoxColor(0), mBoxIB(0), mFX(0), mTech(0),
...
void BoxApp::DrawScene()
{
	...
	ID3D11Buffer* Buffers[2] = { mBoxXYZ, mBoxColor }; // 위치와 컬러를 각자 다른 인풋 색인에 할당하여 이렇게 해줌
	UINT strides[2] = { sizeof(XMFLOAT3), sizeof(XMFLOAT4) };
	UINT offsets[2] = { 0,0 };
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		// 36개의 박스 인덱스 버퍼들
		md3dImmediateContext->DrawIndexed(36, 0, 0); // 정점 인덱스 개수, 첫 정점 위치, 정점에 더해질 값을 지정해 실제로 그리기
	}
...
void BoxApp::BuildGeometryBuffers() // 정점 정의 함수
{
	...
	D3D11_BUFFER_DESC vbd, vbd2;
	D3D11_SUBRESOURCE_DATA vinitData, vinitData2;
	// 보통은 D3D11_USAGE_DEFAULT를 해서 GPU가 자원을 읽고 쓰게 하지만 
	// 여기선 최적화를 위해 버퍼 생성 후에 전혀 변경이 없는 D3D11_USAGE_IMMUTABLE를 사용
	vbd.Usage = D3D11_USAGE_IMMUTABLE;
	vbd.ByteWidth = sizeof(XMFLOAT3) * 8; // 생성할 정점 버퍼의 크기
	...
	vinitData.pSysMem = Pos;
	HR(md3dDevice->CreateBuffer(&vbd, &vinitData, &mBoxXYZ)); // vbd(정점 버퍼 속성), vinitData(정점 초기화), mBoxXYZ(생성된 정점이 담김)
	...
	vbd2.ByteWidth = sizeof(XMFLOAT4) * 8; // 생성할 정점 버퍼의 크기
	vbd2.BindFlags = D3D11_BIND_VERTEX_BUFFER; // 정점 버퍼는 무조건 이거 쓴다.
	vbd2.CPUAccessFlags = 0; // CPU가 정점 버퍼 계산에 관여하지 않으므로 0
	vbd2.MiscFlags = 0; // 정점 버퍼에서는 이건 무조건 0
	vbd2.StructureByteStride = 0; // 구조적 버퍼 사용 안하면 0
	vinitData2.pSysMem = Colors;
	HR(md3dDevice->CreateBuffer(&vbd2, &vinitData2, &mBoxColor));
...
void BoxApp::BuildVertexLayout()
{
	...
	// , 나머지 2개의 속성은 일단 고정
	// 간단히 어떤 속성이 어떤 형태로 어떤 인풋 색인이 할당되고 어떤 이름을 가지고 있는지 설정해주는 것이다.
