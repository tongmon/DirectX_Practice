...
void TreeBillboardApp::BuildCylinderBuffer(float Rad, float Height, UINT StackCnt)
{
	...
	D3D11_BUFFER_DESC vbd;
	vbd.Usage = D3D11_USAGE_IMMUTABLE;
	vbd.ByteWidth = sizeof(Vertex::CylinderV) * CyVertCnt;
	vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vbd.CPUAccessFlags = 0;
	vbd.MiscFlags = 0;
	D3D11_SUBRESOURCE_DATA vinitData;
	vinitData.pSysMem = &vert[0];
	HR(md3dDevice->CreateBuffer(&vbd, &vinitData, &mCylinderVB));
}

void TreeBillboardApp::DrawCylinder(CXMMATRIX viewProj)
{
	XMMATRIX world = XMLoadFloat4x4(&mCylinderWorld);
	XMMATRIX Invtranse = MathHelper::InverseTranspose(world);
	XMMATRIX WVP = world * viewProj;
	Effects::CylinderFX->SetDirLights(mDirLights);
	Effects::CylinderFX->SetEyePosW(mEyePosW);
	Effects::CylinderFX->SetFogColor(Colors::Silver);
	Effects::CylinderFX->SetFogStart(15.0f);
	Effects::CylinderFX->SetFogRange(175.0f);
	Effects::CylinderFX->SetWorldViewProj(WVP);
	Effects::CylinderFX->SetWorld(world);
	Effects::CylinderFX->SetWorldInvTranspose(Invtranse);
	Effects::CylinderFX->SetMaterial(mCylinderMat);

	md3dImmediateContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_LINESTRIP);
	md3dImmediateContext->IASetInputLayout(InputLayouts::CylinderV);
	UINT stride = sizeof(Vertex::CylinderV);
	UINT offset = 0;
